package 
{
	...
	import ShapeSorter;  //before I forget...

	public class KinectMagic extends ShapeSorter
	{
		...
		public function KinectMagic():void
		{
		...
		private function on_enterFrame(e:Event):void
		{
			...
			{
				skeletonHolder.graphics.beginFill(0xffffff);

				for each (var joint:MSSkeletonJoint in user.skeletonJoints)
				{

					//trace("joints :"+joint.name);
					trace("left hand is at :"+user.leftHand.position.depth);
					//Draw a circle on all the joints
					skeletonHolder.graphics.drawCircle(joint.position.depth.x, joint.position.depth.y, 5 );
				}
